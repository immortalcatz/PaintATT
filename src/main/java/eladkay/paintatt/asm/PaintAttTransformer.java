...
public class PaintAttTransformer implements IClassTransformer, Opcodes {
    private static final String ASM_HOOKS = "eladkay/paintatt/asm/PaintAttCorePlugin";
    private static final Map<String, Transformer> transformers = new HashMap<>();

    static {
        transformers.put("net.minecraft.block.Block", PaintAttTransformer::transformBlock);
    }

    private static byte[] transformBlock(byte[] basicClass) {
        MethodSignature sig1 = new MethodSignature("getBlockLayer", "func_180664_k",
                "()Lnet/minecraft/util/BlockRenderLayer;");

        MethodSignature sig2 = new MethodSignature("isFullCube", "func_149686_d",
                "(Lnet/minecraft/block/state/IBlockState;)Z");

        MethodSignature sig3 = new MethodSignature("isOpaqueCube", "func_149662_c",
                "(Lnet/minecraft/block/state/IBlockState;)Z");

        byte[] transformedClass = basicClass;
//                transform(basicClass, sig1, "Block Painting Properties", (MethodNode method) -> { // Action
...
//        });
        ...
        transformedClass = transform(transformedClass, sig2, "Block Painting Properties", combine((AbstractInsnNode node) -> node.getOpcode() == RETURN,
                (MethodNode method, AbstractInsnNode node) -> {
                    InsnList newInstructions = new InsnList();
                    newInstructions.add(new VarInsnNode(ALOAD, 0));
                    newInstructions.add(new VarInsnNode(ALOAD, 1));
                    newInstructions.add(new MethodInsnNode(INVOKESTATIC, ASM_HOOKS, "isFullCube", "(Lnet/minecraft/block/Block;Lnet/minecraft/block/state/IBlockState;)Z", false));
                    method.instructions.insertBefore(node, newInstructions);
                    return false;
                }));

        return transformedClass;
//                transform(transformedClass, sig2, "Block Painting Properties", combine((AbstractInsnNode node) -> node.getOpcode() == RETURN,
//                (MethodNode method, AbstractInsnNode node) -> {
//                    InsnList newInstructions = new InsnList();
//                    newInstructions.add(new FieldInsnNode(GETSTATIC, ASM_HOOKS, "INSTANCE", "L" + ASM_HOOKS + ";"));
...
//                }));
    ...
    public static void log(String str) {
        LogManager.getLogger("LibrarianLib ASM (PaintATT)").info(str);
    }
